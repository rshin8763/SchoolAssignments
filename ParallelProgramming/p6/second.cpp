...
{
    for(int num_elements = 1024*1024; num_elements <= 8*1024*1024; num_elements += 1024*1024){
            int num_workgroups = num_elements/LOCAL_SIZE;
            ...
            float *hC = new float[ num_workgroups ];
            ...
            size_t workgroup_size = num_workgroups*sizeof(float);
            ...
            cl_mem dC = clCreateBuffer( context, CL_MEM_WRITE_ONLY, workgroup_size, NULL, &status );
            if( status != CL_SUCCESS )
            ...
            status = clEnqueueReadBuffer( cmdQueue, dC, CL_TRUE, 0, workgroup_size, hC, 0, NULL, NULL );
            if( status != CL_SUCCESS )
            ...
            if (expected != sum) {
                /* fprintf(stderr, "expected %f, got %f!", expected, sum); */
            }

            fprintf( stderr, "%8d\t%4d\t%10d\t%10.3lf GigaMultsPerSecond\n",
                    num_elements, LOCAL_SIZE, num_workgroups, (double)num_elements/(time1-time0)/1000000000. );
