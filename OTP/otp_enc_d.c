...
// This function will encrypt the message to OTP code.
int encrypt(char *buffer, char *key){
    if (strlen(buffer) > strlen(key)) {
        ...
        for (i=0; i<strlen(buffer); i++){
            if (buffer[i] == ' ') b = 26; // deal with special case assignment of spaces
            else b = buffer[i] - 'A';
            if (key[i] == ' ') k = 26;
            else k = key[i] - 'A';
            val = (b + k) % 27;
            ...
            switch (myPid){
                ...
                case 0: {
                            ...
                            if (strcmp(buffer, "enc") != 0) {
                                ...
                                // encrypt message and send appropriate message.
                                if (encrypt(completeMessage, key) == 0)
                                {
                                    if(DEBUG){
                                        printf("Sending file\n");
                                        printStringContents(completeMessage);
